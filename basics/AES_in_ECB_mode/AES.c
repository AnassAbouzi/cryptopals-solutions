...
uint8_t subbyte(uint8_t x) { return sbox[x]; }
uint8_t inv_subbyte(uint8_t y) { return inv_sbox[y]; }

int subbytes(uint8_t *blk, int len) {
	if (!blk) return -1;

	for (int i = 0; i < len; i++) blk[i] = subbyte(blk[i]);
...
int inv_subbytes(uint8_t *blk, int len) {
	...
	for (int i = 0; i < len; i++) blk[i] = inv_subbyte(blk[i]);
...
int shift_rows(uint8_t *blk) {
	...
	tmp = blk[1]; blk[1] = blk[5]; blk[5] = blk[9]; blk[9] = blk[13]; blk[13] = tmp;

	tmp = blk[2]; blk[2] = blk[10]; blk[10] = tmp;
	tmp = blk[6]; blk[6] = blk[14]; blk[14] = tmp;

	tmp = blk[3]; blk[3] = blk[15]; blk[15] = blk[11]; blk[11] = blk[7]; blk[7] = tmp;
...
int inv_shift_rows(uint8_t *blk) {
	...
	tmp = blk[15]; blk[15] = blk[3]; blk[3] = blk[7]; blk[7] = blk[11]; blk[11] = tmp;
...
int mix_columns(uint8_t *blk) {
	...
	uint8_t c[4];
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) c[j] = blk[i*4 + j];

		mix_column(c);

		for (int j = 0; j < 4; j++) blk[i*4 + j] = c[j];
	}
...
int add_round_key(uint8_t *blk, uint8_t *key, int len) {
	...
	for (int i = 0; i < len; i++) blk[i] = blk[i] ^ key[i];

	return 0;
}

int key_schedule(uint8_t *key, uint8_t round_keys[11][BLK_LEN]) {
	if (!key || !round_keys) return -1;

	uint8_t w[4];
	uint8_t round_key[BLK_LEN];

	memcpy(round_keys[0], key, BLK_LEN);
	memcpy(round_key, key, BLK_LEN);

	for (int i = 0; i < 10; i++) {
		memcpy(w, round_key + 12, 4);
		l_shift_cyclic(w, 4, 1);
		subbytes(w, 4);
		w[0] = w[0] ^ RC[i];
		for (int j = 0; j < 4; j++) round_key[j] ^= w[j];
		for (int j = 4; j < BLK_LEN; j++) round_key[j] ^= round_key[j - 4];
		memcpy(round_keys[i + 1], round_key, BLK_LEN);
	}
	return 0;
}

int aes_encrypt_blk(uint8_t *blk, uint8_t *key, uint8_t *ct) {
	if (!blk || !key || !ct) return -1;

	uint8_t round_keys[11][BLK_LEN];
	key_schedule(key, round_keys);

	memcpy(ct, blk, BLK_LEN);

	add_round_key(ct, round_keys[0], BLK_LEN);

	for (int i = 0; i < 9; i++) {
		subbytes(ct, BLK_LEN);
		shift_rows(ct);
		mix_columns(ct);
		add_round_key(ct, round_keys[i + 1], BLK_LEN);
	}

	subbytes(ct, BLK_LEN);
        shift_rows(ct);
        add_round_key(ct, round_keys[10], BLK_LEN);

	return 0;
}

int aes_encrypt_ECB(uint8_t *pt, int len, uint8_t *key, uint8_t *ct) {
	if (!pt || !key || !ct || (len % 16 != 0)) return -1;

	for (int i = 0; i < len; i += 16) {
		aes_encrypt_blk(pt + i, key, ct + i);
	}
...
int aes_decrypt_blk(uint8_t *blk, uint8_t *key, uint8_t *pt) {
	if (!blk || !key || !pt) return -1;

	uint8_t round_keys[11][BLK_LEN];
	key_schedule(key, round_keys);

	memcpy(pt, blk, BLK_LEN);

	add_round_key(pt, round_keys[10], BLK_LEN);
        inv_shift_rows(pt);
	inv_subbytes(pt, BLK_LEN);


	for (int i = 9; i > 0; i--) {
		add_round_key(pt, round_keys[i], BLK_LEN);
		inv_mix_columns(pt);
		inv_shift_rows(pt);
		inv_subbytes(pt, BLK_LEN);
	}

	add_round_key(pt, round_keys[0], BLK_LEN);
